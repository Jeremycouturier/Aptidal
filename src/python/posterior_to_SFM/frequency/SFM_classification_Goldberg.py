...
radical     = '/home/jeremy/Documents/CleanPosteriors/coordinate_change/processed/simulation/Goldberg/'
typ         = 'Goldberg_turbulence'
numbers     = range(100)

delta_min   = -40. #To be chosen by trial and error
delta_max   = 50.
normX_min   = -2.
normX_max   = 10.
color_min   = -6.
...
for number in numbers:
      if (number < 10):
            ...
            zeros = ''
      path    = radical + typ + '/' + typ + '_' + zeros + str(number) + '/' + typ + '_' + zeros + str(number) + '.csv'
      print("Doing simulation n°", number)
      sample  = np.loadtxt(path, dtype = np.float64, delimiter=',', unpack=True)
      ps      = [2] # p such that the resonance is p : p + 1 for the pair in question
      pairs   = [[0, 1]] # The corresponding pair of planets
      N_pairs = len(ps)
      N       = 2
      for pair in range(N_pairs):
            ...
            for i in range(n):
                  ...
                  x1s.append(max(x1, x2))
                  x2s.append(min(x1, x2))
                  nus.append(nu)
            x1s = np.array(x1s)
            x2s = np.array(x2s)
            nus = np.array(nus)
            ...
            if (straight):
                  ...
                  ax1.plot(Ds, x1s, c = color, marker = 'o', markersize = 5, linewidth = 2, linestyle = '-')
                  #ax1.text(Ds[-1], x1s[-1], str(number), fontsize = 10, color = 'black')
      count = count + 1
...
#artisanal_colorbar(color_min, color_max, 'Density $(m_1+m_2)/(V_1+V_2)$ (water $=1$)', [-2., -1.5, -1., -0.5, 0., 0.5], [r'$10^{-2}$', r'$10^{-1.5}$', r'$10^{-1}$', r'$10^{-0.5}$', r'$1$', r'$10^{0.5}$'])
if (straight):
...
if (straight):
      ...
      ldn = [2.2, 2.6, 3.**(0.5)*2.**(2./3.), 3., 3.5, 4., 5., 6., 8., 12., 20., 36., 68., 132., 260.]
      cs = ax1.contour(X, Y, Z, ldn, linewidths = 1.5, colors = ['grey'], linestyles = 'solid', alpha = 0.3)
