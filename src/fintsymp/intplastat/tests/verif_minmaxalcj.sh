...
mkdir DATA_test
../intplastat.x  tests_intplastatminmaxalcj.par >/dev/null || exit 1
...
try {
npla=2$
fminmax_alc="./DATA_test/testj_proc000.minmax_diffalc"$
fmt="%s %g "+9*" %g"$
...
for k=1 to size(q)
{
 ...
 for j=1 to 1 {
    ...
    {
        error("erreur min/max de a_p(1)-a_p(2)  avec angle continu - jacobi");
    };
 };
...
for k=1 to size(q)
{
 ...
 for j=2 to 2 {
    ...
    if ((diffmin>1E-12) || (diffmax>1E-12) || (diffmoy>1E-12)) then 
    {
        error("erreur min/max de l_p(1)-l_p(2) avec angle continu - jacobi");
    };
 };
...
for k=1 to size(q)
{
 ...
 for j=3 to 3 {
    ...
    diffmax=max(abs(minmax_alc[3*(j-1)+3][q]-vmax[j]))$
    diffmoy=max(abs(minmax_alc[3*(j-1)+2][q]-vmoy[j]))$
    if (diffmin>2*pi) then 
    {
        diffmin=max(abs(sin(minmax_alc[3*(j-1)+1][q]-vmin[j])))$
        diffmax=max(abs(sin(minmax_alc[3*(j-1)+3][q]-vmax[j])))$
        diffmoy=max(abs(sin(minmax_alc[3*(j-1)+2][q]-vmoy[j])))$
    };
    if ((diffmin>1E-12) || (diffmax>1E-12) || (diffmoy>1E-12)) then 
    {
        error("erreur min/max de pi_p(1)-pi_p(2) avec angle continu - jacobi");
    };
 };


};
msg "min/max/moy (jacobi) a_p(1)-a_p(2), ... avec angle continu .....................ok";
t=0,10$
...
msg "-----------------------------------------";
msg "-----------------------------------------";
msg "erreur dans la determination du min/max de a_p(1)-a_p(2) avec angle continu - jacobi";
kpla_cur;
diffmin;
diffmax;
diffmoy;
