...
mkdir DATA_test
../intplastat.x  tests_intplastatnaf2j.par >/dev/null  || exit 1
...
npla=4$
nfreq=5$
fnaf_alkhqp="./DATA_test/test_proc000.naf_alkh"$
fmt="%s %g "+6*nfreq*npla*" %g"$
vnumR t_naf_alkhqp,naf_alkhqp[1:6*nfreq*npla]$
...
msg "-----------------------------------------";
msg "-----------------------------------------";
msg "erreur dans la determination du naf alkh (jacobi)";
ci_cur;
kpla_cur;
k_cur;
difffreq;
