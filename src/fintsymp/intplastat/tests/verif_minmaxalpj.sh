...
mkdir DATA_test
../intplastat.x  tests_intplastatminmaxalpj.par >/dev/null || exit 1

#verification des min/max a_p(1)-a_p(2), ...
rm -f extractok.dat
(trip<<__EOF
try {
npla=2$
fminmax_alp="./DATA_test/testj_proc000.minmax_diffalp"$
fmt="%s %g "+15*" %g"$
...
for k=1 to size(q)
{
 ...
 for j=1 to 1 {
    ...
    {
        error("erreur min/max de a_p(1)-a_p(2) [-pi,pi] et [0,2*pi] - jacobi");
    };
 };
...
for k=1 to size(q)
{
 ...
 for j=2 to 3 {
    ...
    if ((diffmin>1E-12) || (diffmax>1E-12) || (diffmoy>1E-12)) then 
    {
        error("erreur min/max de l_p(1)-l_p(2) [-pi,pi] et [0,2*pi] - jacobi");
    };
 };
...
for k=1 to size(q)
{
 ...
 for j=4 to 5 {
    ...
    if ((diffmin>1E-12) || (diffmax>1E-12) || (diffmoy>1E-12)) then 
    {
        error("erreur min/max de pi_p(1)-pi_p(2) - jacobi");
    };
 };


};
msg "min/max/moy (jacobi) a_p(1)-a_p(2), ... [-pi,pi] et [0,2*pi].....................ok";
t=0,10$
...
msg "-----------------------------------------";
msg "-----------------------------------------";
msg "erreur dans la determination du min/max de a_p(1)-a_p(2)";
kpla_cur;
diffmin;
diffmax;
diffmoy;
