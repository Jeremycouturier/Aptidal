...
mkdir DATA_test
../intplastat.x  tests_intplastatminmaxae2h.par >/dev/null || exit 1
...
try {
npla=2$
fminmax_ae2="./DATA_test/test_proc000.minmax_diffae2"$
fmt="%s %g "+9*" %g"$
...
for k=1 to size(q)
{
 ...
 for j=1 to 1 {
    ...
    {
        error("erreur min/max de (a_p(1)-a_p(2))**2");
    };
 };
...
for k=1 to size(q)
{
 ...
 for j=2 to 2 {
    ...
    if ((diffmin>1E-12) || (diffmax>1E-12) || (diffmoy>1E-12)) then 
    {
        error("erreur min/max de (e_p(1)-e_p(2))**2");
    };
 };
...
for k=1 to size(q)
{
 ...
 for j=3 to 3 {
    ...
    if ((diffmin>1E-12) || (diffmax>1E-12) || (diffmoy>1E-12)) then 
    {
        error("erreur min/max de (a_p(1)-a_p(2))**2+(e_p(1)-e_p(2))**2");
    };
 };

};
msg "min/max/moy (a_p(1)-a_p(2))**2, ... .....................ok";
t=0,10$
