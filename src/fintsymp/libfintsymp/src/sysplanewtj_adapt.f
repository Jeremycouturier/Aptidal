...
!!      On herite de la classe t_sysplanewtJ 
!!
!
! history : creation 01/10/2018
!***********************************************************************
#include "realc.f"
...
! seules les interactions newtonniennes sont prises en compte
!***********************************************************************
      module mod_sysplanewtJ_adapt
      
       use mod_sysplanewtJ
       
       integer, parameter :: NDATAJ_SCH = 5 ! nombre de donnees dans le fichier de sortie (cf. add_adapt_out_step  et sysplanewtJ_adapt_write_output)
                                           ! 5 = rtime + temps(s) + E0 +E1 +H
...
!!  
!***********************************************************************
      type, extends(t_sysplanewtJ) :: t_sysplanewtJ_adapt
          private

          type(compensatedsum) :: rtime  !< real time that is integrated by the system at the same time the Keplerian steps are done. 0 by default
          type(compensatedsum) :: E0     !< Initial energy of the system, it is kept constant during the time of integration
          real(TREAL) :: E1     !< Energy scale for the perturbation used in  the renormalization
          
          contains
          
          procedure :: set_plan_mass =>sysplanewtJ_adapt_set_plan_mass ! fixe la masse des planetes  
          procedure :: set_plan_coord => sysplanewtJ_set_plan_coord ! fixe les coordonnees des planetes (meme que celle integree)

          procedure :: pasA =>sysplanewtJ_adapt_pasA
          procedure :: pasB =>sysplanewtJ_adapt_pasB
          procedure :: pasC =>sysplanewtJ_adapt_pasC
          
          procedure :: energie   => sysplanewtJ_adapt_gamma_energie 
          procedure :: energie_with_err   =>                            &
     &                         sysplanewtJ_adapt_gamma_energie_with_err

          procedure :: create_output => sysplanewtJ_adapt_create_output ! creation d'une copie pour effectuer les pas de sortie
          procedure :: write_output  => sysplanewtJ_adapt_write_output
          procedure :: copy_output    => sysplanewtJ_adapt_copy_output ! copie de this dans la copie de sortie

          procedure :: add_adapt_out_step                                &
     &             => sysplanewtJ_adapt_add_adapt_out_step   ! fixe le pas de sortie, la taille du buffer des donnees suplementaires du schema
          
          procedure :: get_adaptativetime                                &
     &             => sysplanewtJ_adapt_get_adaptativetime ! retourne le temps dans le cas d'un schemea adaptatif

          procedure, public :: dumpbin                                            &
     &             => sysplanewtJ_adapt_dumpbin ! dump binaire vers un fichier
          procedure, public :: restorebin                                         &
     &             => sysplanewtJ_adapt_restorebin ! restauration binaire depuis un fichier


      end type t_sysplanewtJ_adapt
      contains
...
!***********************************************************************
      subroutine sysplanewtJ_adapt_set_plan_mass(this, m)
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this !< dummy argument
       ...
       call sysplanewtJ_set_plan_mass(this, m)
       call this%set_graphdotname("SYSPLA Jacobi adaptatif")
      ...
      end  subroutine sysplanewtJ_adapt_set_plan_mass 
...
!***********************************************************************
      subroutine sysplanewtJ_set_plan_coord(this, X, XP)
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this  !< dummy argument
       ...
       call  sysplanewtJ_energie_with_err(this, E0)
       this%E0 = E0
       this%E1 = -this%E1*E0%v
!Setting the initial energy to zero
       this%m_Hinit%v = 0.
       this%m_Hinit%err = 0.
      end  subroutine sysplanewtJ_set_plan_coord  
...
!***********************************************************************
      subroutine sysplanewtJ_adapt_create_output(this, sysout)
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this  !< dummy argument
      ...
      end  subroutine sysplanewtJ_adapt_create_output 
       
!***********************************************************************
!> @brief ecriture de la sortie (appelee par l'integrateur)
!!   a l'iteration donne et au temps donne
!***********************************************************************
      subroutine sysplanewtJ_adapt_write_output(this, it, t)
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this  !< dummy argument
       ...
       real(TREAL), dimension(1:NDATAJ_SCH) :: schema_dataout
       integer j
       
       rtime = this%rtime%v
       call  sysplanewtJ_energie_with_err(this, H)
       HmE0 = H-this%E0
       ...
       do j=1, size(this%m_stepout)
        if (this%isvalidoutput(it,j)) then 
         ...
         if (this%m_kind(j).eq.3) then
          ...
          ! attention, il faut remplir autant d'element que NDATAJ_SCH
       ...
       end  subroutine sysplanewtJ_adapt_write_output

!***********************************************************************
!> @brief copie de this dans la copie de sortie (sysout)
!! en general, sysout = this 
!***********************************************************************
      subroutine sysplanewtJ_adapt_copy_output(this, sysout)
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this  !< dummy argument
      ...
      end  subroutine sysplanewtJ_adapt_copy_output
...
!***********************************************************************
      subroutine sysplanewtJ_adapt_add_adapt_out_step(this, stepout,                &
     &                  buffersize, buffercs)
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this  !< dummy argument
     ...
     &             buffercs, NDATAJ_SCH , 3) 
       
      end  subroutine sysplanewtJ_adapt_add_adapt_out_step  


!***********************************************************************
!> @brief execute le pas A de l'integrateur
!***********************************************************************
      subroutine sysplanewtJ_adapt_pasA(this, mdt)
       use kahansum
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this  !< dummy argument
       real(TREAL), intent(in) :: mdt !<= tau
       
       real(TREAL) :: tau, f_renorm
       type(compensatedsum) :: H0, E0, rtime, diffH0mE0
       !rtime is the real time of the system
       !E0 is the original energy of the system H0 is the current Keplerian energy

       E0 = this%E0
       rtime = this%rtime
              
       call sysplanewtJ_keplerian_energy(this, H0)
       ...
       this%rtime = rtime + tau

       call sysplanewtJ_pasA(this, tau)
      
      end  subroutine sysplanewtJ_adapt_pasA  


!***********************************************************************
!> @brief execute le pas B de l'integrateur
!***********************************************************************
      subroutine sysplanewtJ_adapt_pasB(this, mdt)
       use kahansum
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this  !< dummy argument
      ...
      call sysplanewtJ_perturbation_energy(this, H1)
...
!!      XPplan = XPplan - tau * Acc
      ...
      end  subroutine sysplanewtJ_adapt_pasB 


!***********************************************************************
!> @brief execute le pas C de l'integrateur
!***********************************************************************
      subroutine sysplanewtJ_adapt_pasC(this, mdt)
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this  !< dummy argument
      ...
      end  subroutine sysplanewtJ_adapt_pasC  


!***********************************************************************
!> @brief calcule l'energie de this
!***********************************************************************
      subroutine sysplanewtJ_adapt_gamma_energie_with_err(this, H)
      use kahansum
      implicit none
  ...
  !   &             (-H1+(H1**2+1)**.5))
      ...
      end  subroutine  sysplanewtJ_adapt_gamma_energie_with_err


   !***********************************************************************
!> @brief calcule l'energie de this
!***********************************************************************
      subroutine sysplanewtJ_adapt_gamma_energie(this, H)
      implicit none
       ...
       type(compensatedsum) :: Gamma_comp !< energie calculee
        ...
        call sysplanewtJ_adapt_gamma_energie_with_err(this,Gamma_comp)
        H = Gamma_comp%v
        
      end  subroutine  sysplanewtJ_adapt_gamma_energie 
!***********************************************************************
!> @brief retourne le temps du systeme dans le cas d'un schemea adaptatif
!***********************************************************************
      function sysplanewtJ_adapt_get_adaptativetime(this) result(t)
       implicit none
       class(t_sysplanewtJ_adapt), intent(in):: this  !< dummy argument
       
       real(TREAL) ::t ! temps du systeme
       t  = this%rtime%v
         
      end function  sysplanewtJ_adapt_get_adaptativetime  
...
!***********************************************************************
      subroutine sysplanewtJ_adapt_dumpbin(this, file) 
       use mod_io_dump
       implicit none
       class(t_sysplanewtJ_adapt), intent(in):: this  !< dummy argument
       type(t_io_dump), intent(in) :: file

       call sysplanewtJ_dumpbin(this, file)

       write(file%m_nf) this%rtime 
       write(file%m_nf) this%E0 
       write(file%m_nf) this%E1 
       
      end subroutine  sysplanewtJ_adapt_dumpbin  
...
!***********************************************************************
      subroutine sysplanewtJ_adapt_restorebin(this, file) 
       use mod_io_dump
       implicit none
       class(t_sysplanewtJ_adapt), intent(inout):: this  !< dummy argument
       type(t_io_dump), intent(in) :: file

       call sysplanewtJ_restorebin(this, file)

       read(file%m_nf) this%rtime 
       read(file%m_nf) this%E0 
       read(file%m_nf) this%E1 
 
       end subroutine  sysplanewtJ_adapt_restorebin 

      end module mod_sysplanewtJ_adapt
