...
!***********************************************************************
      subroutine cidist_integfunc(ligneci, filepar)
        ...
        type(t_io_txt_ncomp) ::io_tide
        type(t_syscircummaree),allocatable, target  :: sysC ! systeme en circumbinaire avec effets de maree
        class(t_syscircummaree), pointer  :: psys ! systeme en circumbinaire avec effets de maree
        ...
        real(TREAL), dimension(3) :: vrot_ci ! vecteur instantane de rotation de la planete interne
        real(TREAL) :: Rpla, xi, k20, taue, tau2 ! parametres physiques de la planete interne
...
! Masses normalisees a 1 = (somme des masses des 2 etoiles)
          mstar = mpl(0)+mpl(1)
          factmassorig = mstar
          cG=cG*mstar   ! car cG est exprime en masse solaire via kgauss
          do k=0,nplan
            mpl(k)=mpl(k)/mstar
          enddo
          ...
          taue         = ligneci%m_taue
          tau2         = ligneci%m_tau2
          vrot_ci(1:3) = ligneci%m_vrot_ci(1:3)
...
! --- create the filename of output files  
      ...
      if (filepar%if_maree.eq.1) then
         write(*,*) ' nf_tid ', trim(filepar%nf_tid)
         io_tide = t_io_txt_ncomp("I/O maree")
         call io_tide%set_comp_nb(1+3+5)
         call io_tide%set_name(trim(filepar%nf_tid),63)
         call io_tide%fcreate()
      end if
...
! --- Creation des tableaux
      ...
      if (int_type.eq.'DOPRI'.or.int_type.eq.'ODEX1') then
        write(*,*) 'coord. circumbinaire et dopri/odex1'
      ...
      call psys%set_tides(filepar%if_maree, Rpla, xi, k20, taue, tau2)
      ...
      ! -- Conditions initiales pour les marees
      call psys%set_vrot(vrot_ci)
      call psys%set_znu2()
...
!------------------------------------------------------------
     ...
     &                        io_cart, io_ell, io_tide)
      ...
      type (t_io_txt_ncomp), intent(inout) :: io_tide !< sortie des marees : moment cinetique + deformation
      class(t_syscircummaree), pointer, intent(inout) :: sys  !< systeme planetaire a integrer 
      ...
      ! sortie des marees
      if (filepar%if_maree.eq.1) then
       call sys%add_maree_out_step(n_out,1_8,io_tide%getconsumer())
      endif
