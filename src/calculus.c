...
void SABAn(typ tau, typ T, int output_step, typ * X_old, int n){
      ...
      /******** Integrates the Hamiltonian with a SABAn method for a  ********/
      /******** time T with a timestep tau. Outputs every output_step ********/
      /******** timestep to file pth/SABAn.txt. n is 1 <= n <= 6.     ********/
      /******** The Keplerian part A is integrated exactly, but the   ********/
      /******** perturbation B is integrated with a Ralston method    ********/
      ...
      file = fopen(file_path, "w");
      if (file == NULL){
            fprintf(stderr, "Error : Cannot create or open file SABA%d.txt in function SABAn.\n", n);
            abort();
      }
      ...
      else{fprintf(stderr, "Error: n must be between 1 and 6 in function SABAn.\n");  abort();}

      /******** Checking validity of coefficients. To be removed when robust ********/
      if (fabs(c1 + d1 + c2 + d2 + c3 + d3 + c4 - 1.) > 1.e-15){
            fprintf(stderr, "Error: The sum of the coefficients does not seem to be 1 in function SABAn.\n");
            abort();
      }      
